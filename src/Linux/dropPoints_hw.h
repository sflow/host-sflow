...
HSP_DROPPOINT(==,arp_overlay,) /* CONTROL: Traps NVE-decapsulated ARP packets that reached the overlay network. This is required, for example, when the address that needs to be resolved is a local address */
...
HSP_DROPPOINT(==,uc_loopback,) /* CONTROL: Traps unicast packets that need to be routed through the same layer 3 interface from which they were received. Such packets are routed by the kernel, but also cause it to potentially generate ICMP redirect packets */
HSP_DROPPOINT(==,local_route,) /* CONTROL: Traps unicast packets that hit a local route and need to be locally delivered */
HSP_DROPPOINT(==,external_route,) /* CONTROL: Traps packets that should be routed through an external interface (e.g., management interface) that does not belong to the same device (e.g., switch ASIC) as the ingress interface */
HSP_DROPPOINT(==,ipv6_uc_dip_link_local_scope,) /* CONTROL: Traps unicast IPv6 packets that need to be routed and have a destination IP address with a link-local scope (i.e., fe80::/10). The trap allows device drivers to avoid programming link-local routes, but still receive packets for local delivery */
...
HSP_DROPPOINT(==,ipv4_router_alert,) /* CONTROL: Traps IPv4 packets that need to be routed and include the Router Alert option. Such packets need to be locally delivered to raw sockets that have the IP_ROUTER_ALERT socket option set */
HSP_DROPPOINT(==,ipv6_router_alert,) /* CONTROL: Traps IPv6 packets that need to be routed and include the Router Alert option in their Hop-by-Hop extension header. Such packets need to be locally delivered to raw sockets that have the IPV6_ROUTER_ALERT socket option set */
HSP_DROPPOINT(==,ptp_event,) /* CONTROL: Traps PTP time-critical event messages (Sync, Delay_req, Pdelay_Req and Pdelay_Resp) */
HSP_DROPPOINT(==,ptp_general,) /* CONTROL: Traps PTP general messages (Announce, Follow_Up, Delay_Resp, Pdelay_Resp_Follow_Up, management and signaling) */
