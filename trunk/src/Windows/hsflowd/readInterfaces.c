 ...
 * and extracts the first IPv4 address which is used to populate adapter->ipAddr.
 * TODO handle IPv6 addresses too.
 */
			...
			if (WBEM_S_NO_ERROR == hr && addresses.vt == (VT_ARRAY |VT_BSTR))  {
				...
				if (SUCCEEDED(hr)) {
					for (LONG idx=lstart; idx <= lend; idx++) {		
						...
						IN_ADDR addr = {0};
						LPCWSTR terminator;
						LONG result = RtlIpv4StringToAddressW(addrStr, TRUE, &terminator, &addr);
						if (NO_ERROR == result) {
								adaptor->ipAddr.addr = addr.S_un.S_addr;
							break;
						} //else try with RtlIpv6StringToAddressW
					}
