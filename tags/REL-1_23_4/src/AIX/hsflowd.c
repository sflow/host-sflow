...
#define SETMYLIMIT(L,V) setMyLimit((L), STRINGIFY(L), (V))
  ...
  */
  ...
  int main(int argc, char *argv[])
  {
    ...
    // on AIX we can get the UUID from here - which makes it easier to
    // use mkssys(1) for scheduling rather than init-scripts
    if(getosuuid((uuid_t *)&sp->uuid, GETOSUUID_AIX) == -1) {
      myLog(LOG_ERR, "getosuuid() failed : %s", strerror(errno));
    }
