...
extern double load_1, load_5, load_15;
  ...
  */
  ...
  int readCpuCounters(SFLHost_cpu_counters *cpu) {
	...
	cpu->cpu_user = (uint32_t)(readSingleCounter("\\Processor(_Total)\\% User Time")/tick_to_ms);
	cpu->cpu_system = (uint32_t)(readSingleCounter("\\Processor(_Total)\\% Privileged Time")/tick_to_ms);
	cpu->cpu_idle = (uint32_t)(readSingleCounter("\\Processor(_Total)\\% Idle Time")/tick_to_ms);
	cpu->cpu_intr = (uint32_t)(readSingleCounter("\\Processor(_Total)\\% Interrupt Time")/tick_to_ms);
